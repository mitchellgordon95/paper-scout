...
export const endorsePaper = functions.https.onCall(async (data, context) => {
  ...
  if (!context?.auth?.uid) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Must be logged in to endorse papers'
    )
  }
  ...
  const paperInfo = (await fetchAndParseArxiv(`id_list=${arxivId}`))
    .pageResults[0]
  ...
  const currentEndorsements = userEndorsements.filter(
    (endorsement) => !endorsement.deletedAt
  )

  if (currentEndorsements.length > 4) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Too many endorsements. Please remove one via your profile page.'
    )
  }

  if (
    userEndorsements.filter((endorsement) => endorsement.paperId === paperId)
      .length > 0
  ) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Paper already or previously endorsed.'
    )
  }

  // Give users points
  const paperEndorsements = (
    await db.collection('endorsements').where('paperId', '==', paperId).get()
  ).docs.map((doc) => doc.data())
  paperEndorsements
    .filter((endorsement) => !endorsement.deletedAt)
    .map((endorsement) => {
      db.collection('users')
...
export const removeEndorsement = functions.https.onCall(
  async (data, context) => {
    const { paperId } = data
    ...
    if (!context?.auth?.uid) {
      throw new functions.https.HttpsError(
        'failed-precondition',
        'Must be logged in to remove endorsements.'
      )
    }
    const userId = context.auth.uid
    ...
    db.collection('papers')
      .doc(paperId)
      .update({ currentEndorsements: decrement })
  }
)
